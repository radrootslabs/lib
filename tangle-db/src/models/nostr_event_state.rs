...
pub fn find_one<E: SqlExecutor>(
    ...
    let result = match opts {
        ...
        INostrEventStateFindOne::Rel(args) => find_one_by_rel(exec, &args.rel)?,
    };
    Ok(IResult { result })
}
...
fn rel_query(rel: &NostrEventStateFindManyRel) -> (&'static str, Vec<Value>) {
    match *rel {}
}

fn find_one_by_rel<E: SqlExecutor>(
    exec: &E,
    rel: &NostrEventStateFindManyRel,
) -> Result<Option<NostrEventState>, IError<SqlError>> {
