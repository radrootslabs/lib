...
#[cfg(feature = "events")]
use crate::events::deletion::RadrootsNostrNip09DeletionRequestEventBuilder;
#[cfg(feature = "events")]
...
impl RadrootsNostrClient {
    ...
    ///
    /// Kind 0 profiles, all kind 1 events, all kind 5 deletion requests, kind
    /// 1111 Comments, and focused or mixed kind 30402 FoodAvailability marker
    /// partitions are rejected because their product shape must come from
    /// typed authoring. Marker-free NIP-99 and operational-only kind 30402
    /// builders remain available for compatibility.
    ...
    /// Publishes a validated NIP-09 deletion request.
    ///
    /// Publication establishes no target-deletion authorization or effect.
    #[cfg(feature = "events")]
    pub async fn send_nip09_deletion_request_event_builder(
        &self,
        event: RadrootsNostrNip09DeletionRequestEventBuilder,
    ) -> Result<RadrootsNostrOutput<RadrootsNostrEventId>, RadrootsNostrError> {
        Ok(self
            .inner
            .send_event_builder(event.into_event_builder())
...
/// Publication establishes no target-deletion authorization or effect.
#[cfg(feature = "events")]
pub async fn radroots_nostr_send_nip09_deletion_request_event(
    client: &RadrootsNostrClient,
    event: RadrootsNostrNip09DeletionRequestEventBuilder,
) -> Result<RadrootsNostrOutput<RadrootsNostrEventId>, RadrootsNostrError> {
    client
        .send_nip09_deletion_request_event_builder(event)
        .await
}
...
mod tests {
    ...
    async fn generic_builder_rejects_all_typed_authoring_reservations_before_signer_access() {
        ...
        let builders = vec![
            ...
            RadrootsNostrGenericEventBuilder::new(deletion_request, "Raw deletion request"),
