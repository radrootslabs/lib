...
fn same_dimension_matches_mass_and_volume_groups() {
    use Unit::*;
    assert!(Unit::same_dimension(MassKg, MassG));
    assert!(Unit::same_dimension(VolumeL, VolumeMl));
    assert!(Unit::same_dimension(Each, Each));
    assert!(!Unit::same_dimension(MassKg, VolumeL));
    assert!(!Unit::same_dimension(Each, MassG));
}
...
fn serde_roundtrip_for_unit_paths() {
    ...
    let back: Unit = serde_json::from_str("\"kg\"").unwrap();
    assert_eq!(back, Unit::MassKg);
    let unknown_err = serde_json::from_str::<Unit>("\"bogus\"").unwrap_err();
    assert!(unknown_err.to_string().contains("unknown unit"));
    let err = serde_json::from_str::<Unit>("123").unwrap_err();
    assert!(err.to_string().contains("invalid type"));
    let missing_err = serde_json::from_str::<Unit>("{}").unwrap_err();
