...
fn execution_from_sp1_public_values(
    ...
    let decoded: radroots_trade_sp1_guest::RadrootsSp1TradeProofPublicValues =
    ...
    let canonical_public_values = radroots_trade_sp1_guest::canonical_public_values_bytes(&decoded)
        .map_err(|error| RadrootsSp1TradeHostError::Sp1PublicValuesDecode(error.to_string()))?;
    let public_values_hash = radroots_trade_sp1_guest::public_values_hash_hex(&decoded)?;
    Ok((
        committed_public_values,
...
mod tests {
    ...
    fn enum_labels_and_display_surfaces_are_complete() {
        ...
        assert_eq!(
            RadrootsSp1TradeHostError::from(
                radroots_trade_sp1_guest::RadrootsSp1TradeGuestError::UnsupportedProofTarget
            ),
    ...
    #[cfg(feature = "sp1_verify")]
    #[test]
    fn legacy_remote_proof_fixture_is_rejected_without_validator_set_binding() {
        let fixture: serde_json::Value = serde_json::from_str(include_str!(
            "../tests/fixtures/remote_returned_proof_order_acceptance_core_v1.json"
        ))
        .expect("legacy remote returned proof fixture JSON");
        let witness = fixture
            .pointer("/remote_prover_request/witness")
            .cloned()
            .expect("legacy witness");
        let error = serde_json::from_value::<RadrootsSp1TradeOrderAcceptanceWitness>(witness)
            .expect_err("legacy proof must fail closed without validator-set binding");
    ...
    fn deterministic_crates_do_not_depend_on_sp1_sdk() {
        ...
        for crate_dir in ["event", "trade", "trade_sp1_guest"] {
