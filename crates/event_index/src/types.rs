#[cfg(not(feature = "std"))]
use alloc::string::String;

#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct RadrootsEventIndexShardId(pub String);

#[cfg_attr(feature = "dto-bindgen", derive(dto_bindgen::Dto))]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct RadrootsEventIndexIdRange {
    pub start: String,
    pub end: String,
}

impl RadrootsEventIndexIdRange {
    pub fn is_valid(&self) -> bool {
        if self.start.is_empty() || self.end.is_empty() {
            return false;
        }
        if self.start.len() != self.end.len() {
            return false;
        }
        if !self.start.chars().all(|c| c.is_ascii_hexdigit())
            || !self.end.chars().all(|c| c.is_ascii_hexdigit())
        {
            return false;
        }
        self.start <= self.end
    }
}

#[cfg(test)]
mod tests {
    use super::RadrootsEventIndexIdRange;
    #[cfg(not(feature = "std"))]
    use alloc::string::String;
    #[cfg(feature = "std")]
    use std::string::String;

    #[test]
    fn id_range_rejects_non_hex() {
        let range = RadrootsEventIndexIdRange {
            start: String::from("zz"),
            end: String::from("ff"),
        };
        assert!(!range.is_valid());
    }

    #[test]
    fn id_range_rejects_mismatched_length() {
        let range = RadrootsEventIndexIdRange {
            start: String::from("0a"),
            end: String::from("0aa"),
        };
        assert!(!range.is_valid());
    }

    #[test]
    fn id_range_rejects_reverse_order() {
        let range = RadrootsEventIndexIdRange {
            start: String::from("0f"),
            end: String::from("0a"),
        };
        assert!(!range.is_valid());
    }

    #[test]
    fn id_range_accepts_hex_order() {
        let range = RadrootsEventIndexIdRange {
            start: String::from("0a"),
            end: String::from("0f"),
        };
        assert!(range.is_valid());
    }

    #[test]
    fn id_range_rejects_empty_bounds() {
        let range = RadrootsEventIndexIdRange {
            start: String::new(),
            end: String::from("0f"),
        };
        assert!(!range.is_valid());
    }

    #[test]
    fn id_range_rejects_empty_end() {
        let range = RadrootsEventIndexIdRange {
            start: String::from("0f"),
            end: String::new(),
        };
        assert!(!range.is_valid());
    }

    #[test]
    fn id_range_rejects_non_hex_end() {
        let range = RadrootsEventIndexIdRange {
            start: String::from("0f"),
            end: String::from("zz"),
        };
        assert!(!range.is_valid());
    }
}
