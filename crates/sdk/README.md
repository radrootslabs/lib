...
`client.storage_operations()` exposes actual owner backup capture, verification,
and finalization through the existing storage SPI. SQLite requires an explicit
host-owned backup root in its open options. Unsupported backends refuse these
operations even if they support reliability metadata. Related application
state, identity binding, media leases, and export consent remain host concerns;
