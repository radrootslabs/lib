...
pub struct RadrootsNostrdbFilterSpec {
...
impl RadrootsNostrdbFilterSpec {
    pub fn new() -> Self {
        Self::default()
    }
...
pub(crate) fn parse_hex_32(
    value: &str,
    field: &'static str,
) -> Result<[u8; 32], RadrootsNostrdbError> {
    let bytes = hex::decode(value).map_err(|source| RadrootsNostrdbError::InvalidHex {
        field,
        reason: source.to_string(),
    })?;

    if bytes.len() != 32 {
        return Err(RadrootsNostrdbError::InvalidHexLength {
            field,
            expected: 32,
            actual: bytes.len(),
        });
...
mod tests {
    ...
    fn filter_spec_builders_and_accessors_round_trip() {
        ...
        let empty_notes = RadrootsNostrdbFilterSpec::text_notes(None, None);
        assert_eq!(empty_notes.kinds(), &[1]);
        assert_eq!(empty_notes.limit(), None);
        assert_eq!(empty_notes.since_unix(), None);

        let spec = RadrootsNostrdbFilterSpec::text_notes(Some(50), Some(100))
    ...
    #[test]
    fn to_nostrdb_filter_builds_supported_success_paths() {
        let event_id = valid_hex_32(0x11);
        let author = valid_hex_32(0x22);

        let _ = RadrootsNostrdbFilterSpec::new()
    ...
    fn parse_hex_32_validates_input() {
        ...
        assert!(matches!(
            invalid_hex,
            Err(RadrootsNostrdbError::InvalidHex { field: "value", .. })
        ));

        let invalid_len = parse_hex_32("abcd", "value");
        assert!(matches!(
            invalid_len,
            Err(RadrootsNostrdbError::InvalidHexLength {
                field: "value",
                expected: 32,
                ..
            })
    ...
    #[test]
    fn to_nostrdb_filter_rejects_invalid_event_id_and_author_hex() {
        let bad_event_id = RadrootsNostrdbFilterSpec::new().with_event_id_hex("not-hex");
        let bad_event_result = bad_event_id.to_nostrdb_filter();
        assert!(matches!(
            bad_event_result,
            Err(RadrootsNostrdbError::InvalidHex {
                field: "event_id",
                ..
            })
        ));

        let bad_author = RadrootsNostrdbFilterSpec::new().with_author_hex("not-hex");
        let bad_author_result = bad_author.to_nostrdb_filter();
        assert!(matches!(
            bad_author_result,
            Err(RadrootsNostrdbError::InvalidHex {
                field: "author",
