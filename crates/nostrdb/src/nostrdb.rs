...
use crate::error::RadrootsNostrdbError;
use crate::filter::parse_hex_32;
use crate::ingest::RadrootsNostrdbIngestSource;
use crate::query::{RadrootsNostrdbNote, RadrootsNostrdbProfile, RadrootsNostrdbQuerySpec};
...
) -> Result<Option<T>, RadrootsNostrdbError> {
    match result {
...
impl RadrootsNostrdb {
    fn serialize_event(event: &RadrootsNostrEvent) -> Result<String, RadrootsNostrdbError> {
        #[cfg(test)]
        if test_hooks::take(&test_hooks::FORCE_EVENT_JSON_ERROR) {
            return Err(RadrootsNostrdbError::EventJsonEncode(
    ...
    ) -> Result<nostrdb::Subscription, RadrootsNostrdbError> {
        #[cfg(test)]
        if test_hooks::take(&test_hooks::FORCE_SUBSCRIBE_ERROR) {
            return Err(RadrootsNostrdbError::Nostrdb(
                "forced subscribe error".into(),
            ));
        }
    ...
    async fn wait_for_notes_inner(
        ...
        subscription: nostrdb::Subscription,
        max_notes: u32,
    ) -> Result<Vec<nostrdb::NoteKey>, RadrootsNostrdbError> {
        #[cfg(test)]
        if test_hooks::take(&test_hooks::FORCE_WAIT_ERROR) {
            return Err(RadrootsNostrdbError::Nostrdb("forced wait error".into()));
        }
        self.inner
    ...
    fn open_txn(&self) -> Result<nostrdb::Transaction, RadrootsNostrdbError> {
        #[cfg(test)]
    ...
    fn query_inner<'a>(
        ...
        filters: &[nostrdb::Filter],
        max_results: i32,
    ) -> Result<Vec<nostrdb::QueryResult<'a>>, RadrootsNostrdbError> {
        #[cfg(test)]
        if test_hooks::take(&test_hooks::FORCE_QUERY_ERROR) {
            return Err(RadrootsNostrdbError::Nostrdb("forced query error".into()));
        }
        self.inner
    ...
    fn note_json_value(note: &nostrdb::Note) -> Result<String, RadrootsNostrdbError> {
        #[cfg(test)]
        if test_hooks::take(&test_hooks::FORCE_NOTE_JSON_ERROR) {
            return Err(RadrootsNostrdbError::Nostrdb(
                "forced note json error".into(),
            ));
        }
    ...
    fn get_profile_record<'a>(
        ...
        txn: &'a nostrdb::Transaction,
        pubkey: &[u8; 32],
    ) -> Result<Option<nostrdb::ProfileRecord<'a>>, RadrootsNostrdbError> {
        #[cfg(test)]
    ...
    pub fn open(config: RadrootsNostrdbConfig) -> Result<Self, RadrootsNostrdbError> {
        ...
        let db_dir_str = db_dir.to_str().ok_or(RadrootsNostrdbError::NonUtf8Path)?;
    ...
    pub fn ingest_event_json_with_source(
        ...
        let metadata = source.to_nostrdb_metadata();
        self.process_event_with_inner(json, metadata)?;
        Ok(())
    }

    pub fn ingest_event_json(&self, json: &str) -> Result<(), RadrootsNostrdbError> {
        self.ingest_event_json_with_source(json, RadrootsNostrdbIngestSource::default())
    }
    ...
    pub fn subscribe(
        &self,
        spec: &RadrootsNostrdbSubscriptionSpec,
    ) -> Result<RadrootsNostrdbSubscriptionHandle, RadrootsNostrdbError> {
        let filters = spec
            .filters()
            .iter()
            .map(|filter_spec| filter_spec.to_nostrdb_filter())
            .collect::<Result<Vec<_>, _>>()?;
        let subscription = self.subscribe_inner(filters.as_slice())?;
        Ok(RadrootsNostrdbSubscriptionHandle::new(subscription.id()))
    }

    pub fn unsubscribe(
        &self,
        handle: RadrootsNostrdbSubscriptionHandle,
    ) -> Result<(), RadrootsNostrdbError> {
    ...
    pub async fn wait_for_note_keys(
        ...
        handle: RadrootsNostrdbSubscriptionHandle,
        max_notes: u32,
    ) -> Result<Vec<RadrootsNostrdbNoteKey>, RadrootsNostrdbError> {
        let note_keys = self
    ...
    pub fn subscription_stream(
        ...
        RadrootsNostrdbSubscriptionStream { inner: stream }
    }

    pub fn query_notes(
        &self,
        spec: &RadrootsNostrdbQuerySpec,
    ) -> Result<Vec<RadrootsNostrdbNote>, RadrootsNostrdbError> {
        ...
        query_results
            ...
            .collect::<Result<Vec<_>, RadrootsNostrdbError>>()
    }

    pub fn get_profile_by_pubkey_hex(
        &self,
        pubkey_hex: &str,
    ) -> Result<Option<RadrootsNostrdbProfile>, RadrootsNostrdbError> {
        ...
        Ok(profile.map(|profile| RadrootsNostrdbProfile {
            profile_key,
...
mod tests {
    ...
    fn config_builder_tracks_values() {
        let config = RadrootsNostrdbConfig::new("target/testdbs/nostrdb_config")
    ...
    fn open_creates_database() {
        ...
        assert_eq!(nostrdb.db_dir(), db_dir.as_path());
    ...
    fn open_rejects_non_utf8_path() {
        ...
        let path = std::path::PathBuf::from(std::ffi::OsStr::from_bytes(b"nostrdb-\xFF"));
        let config = RadrootsNostrdbConfig::new(&path);
        let err = RadrootsNostrdb::open(config).expect_err("non utf8 path");
        assert!(err.to_string().contains("utf-8"));
    }

    #[test]
    fn open_reports_nostrdb_error_for_file_path() {
        let tmp_dir = TempDir::new().expect("tempdir should open");
        let db_dir = tmp_dir.path().join("nostrdb");
        std::fs::write(&db_dir, "not a directory").expect("write db file");
        let config = RadrootsNostrdbConfig::new(&db_dir);
        let err = RadrootsNostrdb::open(config).expect_err("file path should fail");
    ...
    fn ingest_source_builders_track_origin() {
        assert_eq!(
            RadrootsNostrdbIngestSource::default(),
            RadrootsNostrdbIngestSource::client()
        );
        assert_eq!(
            RadrootsNostrdbIngestSource::relay("wss://radroots.org"),
            RadrootsNostrdbIngestSource::Relay {
                relay_url: Some("wss://radroots.org".into())
            }
        );
        assert_eq!(
            RadrootsNostrdbIngestSource::relay_unknown(),
            RadrootsNostrdbIngestSource::Relay { relay_url: None }
        );
    }
    ...
    fn ingest_event_json_rejects_invalid_json() {
        ...
        let err = nostrdb
            .ingest_event_json_with_source("not json", RadrootsNostrdbIngestSource::client())
    ...
    fn subscribe_poll_and_unsubscribe_round_trip() {
        ...
        let config = RadrootsNostrdbConfig::new(&db_dir);
        let nostrdb = RadrootsNostrdb::open(config).expect("database should open");
        let spec = RadrootsNostrdbSubscriptionSpec::single(
            RadrootsNostrdbFilterSpec::new().with_kind(1).with_limit(10),
        );
        ...
        let event = RadrootsNostrEventBuilder::text_note("subscription test")
            ...
            .expect("event should sign");
        nostrdb
            .ingest_event(&event, RadrootsNostrdbIngestSource::relay_unknown())
            .expect("ingest should succeed");

        let mut notes = Vec::new();
        for _ in 0..40 {
            notes = nostrdb.poll_for_note_keys(handle, 32);
            if !notes.is_empty() {
                break;
            }
    ...
    #[test]
    fn subscribe_reports_nostrdb_error() {
        let _guard = test_hooks_guard();
        reset_test_flags();
        let tmp_dir = TempDir::new().expect("tempdir should open");
        let db_dir = tmp_dir.path().join("nostrdb");
        let config = RadrootsNostrdbConfig::new(&db_dir);
        let nostrdb = RadrootsNostrdb::open(config).expect("database should open");
        let spec = RadrootsNostrdbSubscriptionSpec::text_notes(Some(10), None);
        test_hooks::FORCE_SUBSCRIBE_ERROR.store(true, Ordering::SeqCst);

        let err = nostrdb
            .subscribe(&spec)
            .expect_err("forced subscribe error");
        assert!(err.to_string().starts_with("nostrdb error:"));
    }

    #[test]
    fn unsubscribe_reports_nostrdb_error() {
        let _guard = test_hooks_guard();
        reset_test_flags();
        let tmp_dir = TempDir::new().expect("tempdir should open");
        let db_dir = tmp_dir.path().join("nostrdb");
    ...
    #[tokio::test]
    #[allow(clippy::await_holding_lock)]
    async fn wait_for_note_keys_reports_nostrdb_error() {
        let _guard = test_hooks_guard();
        reset_test_flags();
        let tmp_dir = TempDir::new().expect("tempdir should open");
        let db_dir = tmp_dir.path().join("nostrdb");
    ...
    fn query_notes_returns_ingested_results() {
        ...
        let query_spec = RadrootsNostrdbQuerySpec::text_notes(Some(50), None, 50);
        let mut notes = Vec::new();
        for _ in 0..40 {
            notes = nostrdb
    ...
    fn query_notes_empty_filters_returns_empty() {
        ...
        let query_spec = RadrootsNostrdbQuerySpec::new(Vec::new(), 10);
        let notes = nostrdb
            .query_notes(&query_spec)
            .expect("query should succeed");
        assert!(notes.is_empty());
    ...
    fn query_notes_rejects_invalid_filters() {
        ...
        let spec = RadrootsNostrdbQuerySpec::single(
            RadrootsNostrdbFilterSpec::new().with_author_hex("not-hex"),
            10,
        );
        let err = nostrdb.query_notes(&spec).expect_err("invalid filter");
    ...
    fn query_notes_reports_transaction_error() {
        ...
        let config = RadrootsNostrdbConfig::new(&db_dir);
        let nostrdb = RadrootsNostrdb::open(config).expect("database should open");
        let spec = RadrootsNostrdbQuerySpec::text_notes(Some(10), None, 10);
    ...
    fn query_notes_reports_query_error() {
        ...
        let err = nostrdb.query_notes(&spec).expect_err("forced query error");
        assert!(err.to_string().starts_with("nostrdb error:"));
    }
    ...
    fn profile_lookup_returns_metadata_fields() {
        ...
        nostrdb
            .ingest_event(&metadata_event, RadrootsNostrdbIngestSource::client())
            .expect("ingest should succeed");
    ...
    fn profile_lookup_invalid_metadata_content_returns_none() {
        ...
        let result = nostrdb.get_profile_by_pubkey_hex(pubkey_hex.as_str());
    ...
    fn subscribe_rejects_invalid_author_hex() {
        ...
        let err = nostrdb.subscribe(&spec).expect_err("subscribe should fail");
    ...
    async fn subscription_stream_yields_events() {
        ...
        let spec = RadrootsNostrdbSubscriptionSpec::text_notes(Some(10), None);
        let handle = nostrdb.subscribe(&spec).expect("subscribe should succeed");
        let mut stream = nostrdb.subscription_stream(handle, 0);
    ...
    fn concurrent_ingest_handles_parallel_writers() {
        ...
        let query_spec = RadrootsNostrdbQuerySpec::text_notes(Some(512), None, 512);
    ...
    fn giftwrap_secret_key_hex_validates_length() {
        ...
        let result = nostrdb.add_giftwrap_secret_key_hex("abcd");
    ...
    fn giftwrap_process_flow_executes() {
        ...
        let _ = nostrdb.add_giftwrap_secret_key(secret_key);
        nostrdb
            .process_giftwraps()
