...
Phase 1 outbox publication preserves and retries the exact signature-verified
NIP-01 event JSON object bytes persisted by signing. Its stable dispatch
identity excludes attempts, leases, clocks, and process identity. This byte
identity does not extend to WebSocket frames: an adapter may reserialize the
event while constructing `["EVENT", <event>]`, and WebSocket fragmentation is
not protocol identity. Captured-adapter conformance therefore proves exact
decoded event-object bytes, not identical frame bytes.
...
evaluated in one coherent event-store snapshot. A fetch scans at most 4,096
...
`RadrootsRelayUrlPolicy::Public` rejects non-canonical and known non-global
literal destinations. The official publish and fetch adapters additionally
resolve every hostname at connect and reconnect, reject the complete answer if
any address is forbidden, and pin that accepted answer through TCP connect
while retaining the original hostname for TLS verification. Parsing a public
relay URL by itself remains syntax validation rather than an SSRF boundary;
custom adapters must enforce equivalent resolution and connection policy.
