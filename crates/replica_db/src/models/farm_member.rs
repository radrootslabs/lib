use radroots_replica_db_schema::farm_member::{
    FarmMember, FarmMemberQueryBindValues, IFarmMemberCreate, IFarmMemberCreateResolve,
    IFarmMemberDelete, IFarmMemberDeleteResolve, IFarmMemberFieldsFilter, IFarmMemberFindMany,
    IFarmMemberFindManyResolve, IFarmMemberFindOne, IFarmMemberFindOneResolve, IFarmMemberUpdate,
...
pub fn find_one(
    ...
    let result = match opts {
        IFarmMemberFindOne::On(args) => find_one_by_on(exec, &args.on)?,
    };
    Ok(ReplicaSchemaResult { result })
}
...
) -> Result<Vec<FarmMember>, ReplicaSchemaError<SqlError>> {
...
) -> Result<Option<FarmMember>, ReplicaSchemaError<SqlError>> {
...
fn select_by_id(
    exec: &dyn SqlExecutor,
    id: &str,
) -> Result<FarmMember, ReplicaSchemaError<SqlError>> {
    let params_json =
...
pub fn delete(
    ...
    let id_for_lookup = match opts {
        IFarmMemberDelete::On(args) => match args.on.primary_key() {
            Some(id) => id,
            None => {
