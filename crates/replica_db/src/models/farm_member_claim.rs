...
) -> Result<Vec<FarmMemberClaim>, ReplicaSchemaError<SqlError>> {
...
) -> Result<Option<FarmMemberClaim>, ReplicaSchemaError<SqlError>> {
...
fn select_by_id(
    exec: &dyn SqlExecutor,
    id: &str,
) -> Result<FarmMemberClaim, ReplicaSchemaError<SqlError>> {
    let params_json =
...
pub fn delete(
    ...
    let id_for_lookup = match opts {
        IFarmMemberClaimDelete::On(args) => match args.on.primary_key() {
            Some(id) => id,
            None => {
