...
) -> Result<Vec<NostrEventHead>, ReplicaSchemaError<SqlError>> {
...
) -> Result<Option<NostrEventHead>, ReplicaSchemaError<SqlError>> {
...
fn select_by_id(
    exec: &dyn SqlExecutor,
    id: &str,
) -> Result<NostrEventHead, ReplicaSchemaError<SqlError>> {
    let params_json =
