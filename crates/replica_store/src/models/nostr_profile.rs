...
) -> Result<Vec<NostrProfile>, ReplicaSchemaError<SqlError>> {
...
) -> Result<Option<NostrProfile>, ReplicaSchemaError<SqlError>> {
...
fn select_by_id(
    exec: &dyn SqlExecutor,
    id: &str,
) -> Result<NostrProfile, ReplicaSchemaError<SqlError>> {
    let params_json =
