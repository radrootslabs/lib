...
`wire::publication::allowlist::allow_phase1_publication_artifact` is the
additive durable-publication gate over that sealed artifact. It consumes the
artifact and returns an opaque allowlisted typestate carrying exactly one of
the same seven leaves: supporting Profile or one of the six root publication
forms. The companion
`allow_phase1_publication_canonical_json` operation reloads persisted canonical
artifact bytes through the same gate. The gate revalidates the artifact, then
reprojects kind `1` with Ask-before-PhotoUpdate-before-Update precedence,
requires the strict typed date/time NIP-52 profiles, and marker-partitions kind
`30402` before accepting focused FoodAvailability. Raw or generic events,
including raw date/time events, replies, comments, deletion requests, calendar
collections and RSVPs, BUD-11 authorizations, ephemeral events, trade,
commerce, groups, and operations cannot produce the sealed input and fail
closed at canonical reload; deferred route/delivery product surfaces are
non-events and likewise fail closed. This allowlist does not change
event-contract registry v7 or grant signing, media readiness, transport, relay,
