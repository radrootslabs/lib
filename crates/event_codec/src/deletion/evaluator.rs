...
use crate::{
    deletion::admission::RadrootsAdmittedNip09DeletionRequestEvent,
...
fn nip01_coordinate(target: &RadrootsSignatureVerifiedEvent) -> Option<RadrootsNip01Coordinate> {
    ...
    RadrootsNip01Coordinate::parse(format!("{kind}:{}:{identifier}", event.author().as_str())).ok()
}
...
mod tests {
    ...
    fn nip09_evaluator_reduction_is_order_and_repeat_invariant() {
        ...
        let lower_address_id =
            core::cmp::min(address_a.event().id(), address_b.event().id()).clone();
