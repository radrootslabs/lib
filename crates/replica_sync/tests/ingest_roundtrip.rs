...
fn seed_source(
    ...
    let owner_pubkey = test_public_key_hex(0x8);
    let _ = unwrap_sql(
        farm_member::create(
            exec,
            &IFarmMemberFields {
...
fn ingest_roundtrip_yields_zero_pending_sync() {
    ...
    for (index, draft) in drafts.iter().enumerate() {
        ...
        let first = radroots_replica_ingest_event(&target, &event).expect("first ingest");
        assert_eq!(first, RadrootsReplicaIngestOutcome::Applied);
        let second = radroots_replica_ingest_event(&target, &event).expect("second ingest");
        if second == RadrootsReplicaIngestOutcome::Skipped {
            skipped += 1;
        }
    }
    assert!(skipped > 0);

    let status = radroots_replica_sync_status(&target).expect("sync status");
    assert_eq!(status.expected_count, drafts.len());
    assert_eq!(status.pending_count, 0);

    let replay = radroots_replica_sync_all(
        &target,
        &RadrootsReplicaSyncRequest {
            farm: RadrootsReplicaFarmSelector {
                id: None,
    ...
    .expect("replay sync");
    assert_eq!(replay.version, RADROOTS_REPLICA_TRANSFER_VERSION);
...
fn sync_all_selector_and_options_paths_are_supported() {
    ...
    let reduced = radroots_replica_sync_all(
        &source,
        &RadrootsReplicaSyncRequest {
            farm: request.farm,
...
fn ingest_reports_transaction_boundary_errors() {
    let exec = SqlxSqliteExecutor::open_memory().expect("db");
    migrations::run_all_up(&exec).expect("migrations");
    let author = test_public_key_hex(0xa);
...
fn ingest_reports_parse_and_state_error_paths_for_all_kinds() {
    ...
    let profile_pubkey = test_public_key_hex(0xa);
    ...
    let farm_pubkey = test_public_key_hex(0xb);
...
fn ingest_reports_query_fail_paths_for_profile_farm_plot_and_list_sets() {
    ...
    let profile_pubkey = test_public_key_hex(0xd);
    ...
    let farm_pubkey = test_public_key_hex(0xe);
    let farm_d_tag = "AAAAAAAAAAAAAAAAAAAAAA";
...
fn ingest_event_paths_cover_profile_farm_plot_and_list_set_variants() {
    ...
    assert_eq!(
        radroots_replica_ingest_event(&exec, &profile_create).expect("profile create"),
        RadrootsReplicaIngestOutcome::Applied
    );
    assert_eq!(
        radroots_replica_ingest_event(&exec, &profile_create).expect("profile skip same"),
    ...
    let err = radroots_replica_ingest_event(&exec, &profile_missing_type)
        .expect_err("profile type is required");
    assert!(err.to_string().contains("profile_type required"));

    let profile_types = [
        (
            RadrootsProfileType::Farm,
            test_public_key_hex(0xf),
            "farm-profile",
        ),
        (
            RadrootsProfileType::Coop,
            test_public_key_hex(0xc),
            "coop-profile",
        ),
        (
            RadrootsProfileType::Any,
            test_public_key_hex(0xa),
            "any-profile",
        ),
        (
    ...
    for (index, (profile_type, pubkey, name)) in profile_types.iter().enumerate() {
        ...
        assert_eq!(
            radroots_replica_ingest_event(&exec, &event).expect("profile variant"),
            RadrootsReplicaIngestOutcome::Applied
        );
    }
    ...
    assert_eq!(
        radroots_replica_ingest_event(&exec, &farm_create).expect("farm skip same"),
        RadrootsReplicaIngestOutcome::Skipped
    );
    ...
    assert_eq!(
        radroots_replica_ingest_event(&exec, &plot_create).expect("plot skip same"),
        RadrootsReplicaIngestOutcome::Skipped
    );
    ...
    let plot_missing_farm = plot_event(
        ...
        "AAAAAAAAAAAAAAAAAAAAAg",
        RadrootsFarmRef {
            pubkey: test_public_key_hex(0x3),
            d_tag: "AAAAAAAAAAAAAAAAAAAAAw".to_string(),
        },
        "plot-missing-farm",
        None,
    ...
    let missing_farm_err = radroots_replica_ingest_event(&exec, &plot_missing_farm)
    ...
    assert_eq!(
        radroots_replica_ingest_event(&exec, &non_generic_event).expect("non-generic list set"),
        RadrootsReplicaIngestOutcome::Skipped
    );
    ...
    let metadata_err = radroots_replica_ingest_event(&exec, &metadata_event)
    ...
    assert_eq!(
        radroots_replica_ingest_event(&exec, &member_of_event).expect("member_of apply"),
        RadrootsReplicaIngestOutcome::Applied
    );
    assert_eq!(
    ...
    let invalid_members_err = radroots_replica_ingest_event(&exec, &invalid_members_event)
    ...
    let members_valid = farm_list_sets::farm_members_list_set(
        farm_d_tag,
        vec![test_public_key_hex(0x6), test_public_key_hex(0x6)],
    )
    .expect("members list");
    ...
    assert_eq!(
        radroots_replica_ingest_event(&exec, &workers_event).expect("workers apply"),
        RadrootsReplicaIngestOutcome::Applied
    );
    ...
    let invalid_plots_err = radroots_replica_ingest_event(&exec, &invalid_plots_event)
    ...
    assert_eq!(
        radroots_replica_ingest_event(&exec, &plots_event).expect("plots apply"),
        RadrootsReplicaIngestOutcome::Applied
    );
    ...
    let unsupported_err = radroots_replica_ingest_event(&exec, &unsupported_event)
...
fn sync_status_reports_pending_when_not_all_events_are_ingested() {
    ...
    for (index, draft) in drafts.iter().enumerate() {
        let event = draft_to_event(draft, index as u32);
        let _ = radroots_replica_ingest_event(&target, &event).expect("ingest");
    }
    target
        .exec(
    ...
    let status = radroots_replica_sync_status(&target).expect("status pending");
    assert_eq!(status.expected_count, drafts.len());
...
fn sync_all_rejects_invalid_selectors_and_resolves_unique_pair() {
    ...
    let missing_id_err = radroots_replica_sync_all(
        &exec,
        &RadrootsReplicaSyncRequest {
            farm: RadrootsReplicaFarmSelector {
    ...
    let duplicate_pubkey = test_public_key_hex(0xe);
...
fn sync_emit_handles_invalid_geojson_and_unknown_profile_type() {
    ...
    let farm_pubkey = test_public_key_hex(0x0);
    let farm_d_tag = "AAAAAAAAAAAAAAAAAAAAAA".to_string();
...
fn error_conversion_paths_are_exercised() {
    let sql: RadrootsReplicaEventsError = ReplicaSchemaError::from(SqlError::Internal).into();
