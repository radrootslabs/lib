...
pub fn generate(
    ...
    let toolchain = resolve_wasm_toolchain(source_root)?;
    println!(
    ...
    for spec in specs {
        let staging = tempfile::tempdir()
            .map_err(|error| format!("failed to create WASM staging directory: {error}"))?;
        let dist_dir = staging.path().join(spec.package_dir).join("dist");
        ...
        write_declaration_files(staging.path(), spec)?;
        write_provenance_manifest(source_root, consumer_root, staging.path(), spec)?;
        validate_staged_outputs(&dist_dir, spec)?;
        install_outputs(source_root, consumer_root, staging.path(), spec, mode)?;
...
fn write_provenance_manifest(
    ...
    let manifest = provenance_manifest(source_root, consumer_root, staged_root, spec)?;
    let dist_dir = staged_root.join(spec.package_dir).join("dist");
...
fn provenance_manifest(
    ...
    let package_version =
        manifest_version(&consumer_root.join(spec.package_dir).join("package.json"))?;
    let rust_crate_version =
        manifest_version(&source_root.join(spec.crate_dir).join("Cargo.toml"))?;
    let source_sha256 = source_tree_sha256(source_root, spec)?;
    let dist_dir = output_root.join(spec.package_dir).join("dist");
...
fn validate_provenance_manifest(
    ...
    let path = provenance_manifest_path(&consumer_root.join(spec.package_dir).join("dist"), spec);
    let raw = fs::read_to_string(&path)
        .map_err(|error| format!("failed to read {}: {error}", path.display()))?;
    let actual: WasmProvenanceManifest = serde_json::from_str(&raw)
        .map_err(|error| format!("failed to parse {}: {error}", path.display()))?;
    let expected = provenance_manifest(source_root, consumer_root, consumer_root, spec)?;
    if actual != expected {
        return Err(format!(
...
fn install_outputs(
    ...
    let staged_dist = staged_root.join(spec.package_dir).join("dist");
    let destination_dist = consumer_root.join(spec.package_dir).join("dist");
    for relative in wasm_output_files(spec)
        .into_iter()
        .chain([format!("{}.provenance.json", spec.out_name)])
    {
        let staged = staged_dist.join(&relative);
        let bytes = fs::read(&staged)
            .map_err(|error| format!("failed to read {}: {error}", staged.display()))?;
        let destination = destination_dist.join(relative);
        match mode {
            Mode::Write => crate::build_control::atomic_write_bytes(&destination, &bytes)?,
            Mode::Check => {
                ...
                if current != bytes {
                    return Err(format!(
                        "stale generated SDK WASM output: {}",
                        destination.display()
                    ));
                }
            }
    ...
    validate_provenance_manifest(source_root, consumer_root, spec)
}

fn validate_staged_outputs(dist_dir: &Path, spec: WasmPackageSpec) -> Result<(), String> {
    let expected = wasm_output_files(spec)
        .into_iter()
        .chain([format!("{}.provenance.json", spec.out_name)])
        .collect::<BTreeSet<_>>();
    let actual = fs::read_dir(dist_dir)
        .map_err(|error| format!("failed to read {}: {error}", dist_dir.display()))?
        .map(|entry| {
            entry
                .map_err(|error| format!("failed to enumerate {}: {error}", dist_dir.display()))
                .and_then(|entry| {
                    entry
                        .file_name()
                        .into_string()
                        .map_err(|_| format!("non-UTF-8 WASM output in {}", dist_dir.display()))
                })
        })
        .collect::<Result<BTreeSet<_>, _>>()?;
    if actual != expected {
        return Err(format!(
            "unexpected WASM output inventory for {}: expected {expected:?}, found {actual:?}",
            spec.package_name
        ));
    }
    Ok(())
...
mod tests {
    ...
    use crate::sdk_generation::package_matrix::wasm_package_specs;
    ...
    #[test]
    #[ignore = "requires wasm-pack, the pinned WASM target, and a WASM C compiler"]
    fn canonical_wasm_generation_round_trips_through_an_explicit_consumer() {
        let consumer = tempfile::TempDir::new().expect("consumer fixture");
        for spec in wasm_package_specs() {
            let package_dir = consumer.path().join(spec.package_dir);
        ...
        super::generate(&source, consumer.path(), &[], Mode::Write).expect("WASM generation");
        super::generate(&source, consumer.path(), &[], Mode::Check).expect("fresh WASM outputs");
    }
